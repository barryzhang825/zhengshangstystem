...
<script type='ecmascript-6'>
  ...
  import user from './user.vue';
    ...
    mounted (){
      ...
      _this.$get('/background-partner/queryByPage',pageData).then(data=>{
    ...
    methods: {
      deleteFun(row){
        ...
        _this.$confirm('是否要删除', '是否继续?', '提示', {
          ...
          type: 'warning'
        	...
        	_this.$get('/background-partner/delete',{id:row.id}).then(data=>{
        		var seller=JSON.stringify(data);
        		_this.$message({
        			type: 'success',
        			message: data+'!'
      ...
      changeSwitch (row) {
        ...
        var _this=this;
          ...
          type: 'warning'
					...
					_this.$get('/background-partner/updateUse',dataStr).then(data=>{
						var seller=JSON.stringify(data);
						//console.log('结果:'+seller);
      ...
      pageAxios(page){
        ...
        _this.$get('/background-partner/queryByPage',dataObject).then(data=>{
          var seller=JSON.stringify(data);
          ////console.log('结果:'+seller);
          _this.ourServiceList=data.list;
          _this.totalPages=data.pages;
