		...
		<div class="modify" v-show="isModify">
          ...
          <div v-show="isHidden" class="confomml-name upload-demo"><label for="serviceId">id</label>{{serviceId}}</div>
					<div class="confomml-name upload-demo ">
						<label for="serviceSort">排序数值</label>
            <input id="serviceSort" name="serviceSort" v-model="serviceSort" type="text" placeholder="请输入排序数值">
					</div>
          <div class="confomml-name upload-demo ">
          	<label for="serviceText">轮播图文字</label>
          	<input id="serviceText" name="serviceText" v-model="serviceText" type="text" placeholder="请输入轮播图文字">
          </div>
...
<script type='ecmascript-6' scoped>
	export default {
    props:{
    },
    data () {
      return {
        upLoadUrl:'http://39.98.80.223:8080/official-website/background-slideshow/update',
        isModify:true,//是否修改
        oldImg:'',
        serviceId:0,
      ...
      change(){
        ...
        //console.log("确认修改")
          ...
          messages: {
            ...
            _this.getfileData.sort=Number(_this.serviceSort);
